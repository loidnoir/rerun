use rerun::{
    external::arrow2, ChunkStore, ChunkStoreConfig, Component, ComponentDescriptor, ComponentName,
    Loggable, VersionPolicy,
};
...
impl Component for CustomPosition3D {
    ...
    fn name() -> ComponentName {
        "user.CustomPosition3D".into()
    }

    #[inline]
    fn descriptor() -> ComponentDescriptor {
        ComponentDescriptor {
            archetype_name: Some("AAA".into()),
            archetype_field_name: Some("BBB".into()),
            component_name: "CCC".into(),
        }
    }
}
    ...
    if let Ok(path_to_rrd) = std::env::var("_RERUN_TEST_FORCE_SAVE") {
        ...
        let expected = vec![
            ComponentDescriptor {
                ...
                component_name: CustomPosition3D::name(),
            }, //
        ];
